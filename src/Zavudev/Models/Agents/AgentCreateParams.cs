...
///
/// <para>**Sub-resources.** An agent's tools, flows and knowledge bases are reachable
/// at `/v1/agents/{agentId}/tools`, `/v1/agents/{agentId}/flows` and `/v1/agents/{agentId}/knowledge-bases`,
/// mirroring the sender-scoped routes documented under `/v1/senders/{senderId}/agent/...`
/// exactly. Use the agent-scoped form while the agent has no sender: the sender-scoped
/// one cannot address it.</para>
///
...
public record class AgentCreateParams : ParamsBase
{
    ...
    /// </summary>
    public required ApiEnum<string, Agent::AgentProvider> Provider
    {
        get
        {
            this._rawBodyData.Freeze();
            return this._rawBodyData.GetNotNullClass<ApiEnum<string, Agent::AgentProvider>>(
                "provider"
            );
        }
    ...
    /// behaves on inbound and outbound phone calls through Zavu's managed voice pipeline
    /// (speech recognition, the agent's LLM, and speech synthesis, with real-time
    /// interruption handling). Requires the Voice Agents feature to be enabled for
    /// your team.
    /// </summary>
    public Voice? Voice
    {
...
/// on inbound and outbound phone calls through Zavu's managed voice pipeline (speech
