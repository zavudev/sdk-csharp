...
public record class ExecutionListParams : ParamsBase
{
    ...
    /// </summary>
    public ApiEnum<string, AgentExecutionStatus>? Status
    {
        get
        {
            this._rawQueryData.Freeze();
            return this._rawQueryData.GetNullableClass<ApiEnum<string, AgentExecutionStatus>>(
                "status"
            );
        }
        init
...
#pragma warning disable CS8618
    ...
    {
        this.SenderID = executionListParams.SenderID;
    }
    ...
    public override Uri Url(ClientOptions options)
    {
        return new UriBuilder(
            options.BaseUrl.ToString().TrimEnd('/')
                + string.Format("/v1/senders/{0}/agent/executions", this.SenderID)
        )
        {
