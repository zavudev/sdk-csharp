...
/// calls reports zeros on every field. Use `GET /v1/calls` for voice activity, duration,
/// and cost.</para>
///
...
/// </summary>
public record class AgentStatsParams : ParamsBase
{
    public string? SenderID { get; init; }

    public AgentStatsParams() { }

#pragma warning disable CS8618
    [SetsRequiredMembers]
    public AgentStatsParams(AgentStatsParams agentStatsParams)
        : base(agentStatsParams)
    {
        this.SenderID = agentStatsParams.SenderID;
    }
...
#pragma warning restore CS8618
    ...
    public static AgentStatsParams FromRawUnchecked(
        IReadOnlyDictionary<string, JsonElement> rawHeaderData,
        IReadOnlyDictionary<string, JsonElement> rawQueryData,
        string senderID
    )
    ...
    public virtual bool Equals(AgentStatsParams? other)
    {
        if (other == null)
        {
            return false;
    ...
    public override Uri Url(ClientOptions options)
    {
        return new UriBuilder(
            options.BaseUrl.ToString().TrimEnd('/')
                + string.Format("/v1/senders/{0}/agent/stats", this.SenderID)
        )
        {
