...
/// within the daily ceiling: 5 calls a day for an account that has verified nothing,
/// 50 a day on Free once it has verified its identity, added a payment method, settled
/// a deposit or subscribed. Paid plans have no daily call ceiling. Over it, `429`
/// with code `daily_limit_exceeded`. Full reference: https://docs.zavu.dev/concepts/sending-limits
...
/// the account's calling, answered with `403` and code `sending_suspended`. - The
/// sender's agent must have `voice.enabled` set to `true`. - Not available with
/// test-mode API keys.</para>
///
