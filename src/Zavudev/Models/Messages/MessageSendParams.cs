...
/// exactly as a message you send them does, so a project that has sent 300 and received
/// 1,700 has used the whole allowance. Messages you send from the WhatsApp Business
/// App on your own phone under coexistence are mirrored into your inbox but never
/// counted, and neither are failed sends. Over the allowance, sends return 429 with
/// code `a2p_limit_exceeded` and upgrade details, **and inbound messages on those
/// channels are refused as well**: not stored, not shown in the inbox, and no `message.inbound`
/// webhook, and not delivered later when the month resets. The counter resets on
/// the 1st of each month. Paid plans have no message caps - Email is billed from
...
/// on every plan</para>
///
/// <para>**Daily limits:** - An account sends on every channel from its first minute,
/// to any destination. Verification is not a permission to send: identity verification
/// and business verification (KYB) raise the ceilings below and nothing else asks
/// for them here. KYB is still required to register a 10DLC brand and campaign, which
/// every US and Canadian (+1) SMS destination needs — a carrier rule, answered separately
/// with `403 ten_dlc_required` - Daily ceilings apply per channel group and rise
/// with verification. An account that has verified nothing: 25/day across `sms` +
/// `sms_oneway`, 5/day for `voice`, 100/day across WhatsApp, Telegram, Instagram
/// and Messenger combined. Past that floor: 200/day for SMS, or 10,000/day once identity
/// or business verification is approved (or a higher limit agreed for your account);
/// 50/day voice and 250/day conversational on Free. **Paid plans have no voice or
/// conversational daily ceiling.** Over a ceiling, sends return `429` with code `daily_limit_exceeded`
/// and `details.limit`; the count resets at 00:00 UTC - The daily ceiling never reduces
/// the monthly allowance: 100/day on the conversational group still reaches the 2,000
/// monthly A2P messages Free includes - Email: a sender with a verified domain sends
/// from day one, within the plan quota (100/day and 3,000/month on Free). Over the
/// daily quota it returns `429` with code `daily_limit_exceeded` - Full reference: https://docs.zavu.dev/concepts/sending-limits</para>
///
...
/// sending. What is checked is the message, not who you are. - A message can be **held**
/// for a short review. It stays `queued` while it waits: no new status exists for
/// this, and `MessageStatus` is unchanged. When it is approved it sends normally.
/// - A message that is not approved moves to `failed` and fires `message.failed`.
/// `errorCode` says which happened: `RISK_REJECTED` (a reviewer refused it), `RISK_REVIEW_EXPIRED`
/// (the review window closed first — it is a couple of hours, because a code that
/// arrives late is worse than one that does not arrive), or `RISK_BLOCKED` (refused
/// outright, without a hold). An SMS that fails this way is not charged; the prepaid
/// amount is returned. - A call is never held. `POST /v1/calls` fails a call the
/// review stops rather than placing it hours late. - A message whose content cannot
/// be read — the check is briefly unavailable — is held rather than sent. An account
/// with an approved business verification is unaffected, and so is one that has
/// verified something, already sends real traffic, and has a clean recent record.
/// - Repeated refusals suspend an account's sending. While it is suspended every
/// send is refused with `403` and code `sending_suspended`, `details.dashboardUrl`
/// points at support, and a message already queued fails with `errorCode` `SENDING_SUSPENDED`.
/// - A broadcast is read once, on the broadcast itself, rather than per recipient
/// — see `POST /v1/broadcasts/{broadcastId}/send`.</para>
///
