...
/// verification (KYB) is required for **one channel only**: `sms_oneway`. Without
/// an approved KYB, one-way SMS returns `403` with code `kyb_required` and `details.dashboardUrl`
/// pointing at `/kyb`, whatever the account has otherwise verified. No other channel
/// asks for it - Daily ceilings apply per channel group and rise with verification.
/// An account that has verified nothing: 25/day across `sms` + `sms_oneway`, 5/day
/// for `voice`, 100/day across WhatsApp, Telegram, Instagram and Messenger combined.
/// Past that floor: 200/day for SMS, or 10,000/day once identity or business verification
/// is approved (or a higher limit agreed for your account); 50/day voice and 250/day
/// conversational on Free. **Paid plans have no voice or conversational daily ceiling.**
/// Over a ceiling, sends return `429` with code `daily_limit_exceeded` and `details.limit`;
/// the count resets at 00:00 UTC - The daily ceiling never reduces the monthly allowance:
/// 100/day on the conversational group still reaches the 2,000 monthly A2P messages
/// Free includes - Email needs no account verification here: a sender with a verified
/// domain sends from day one, within the plan quota (100/day and 3,000/month on
/// Free). Over the daily quota it returns `429` with code `daily_limit_exceeded`.
/// Email broadcasts are the exception: they need the account past the unverified
/// level, see `POST /v1/broadcasts/{broadcastId}/send` - Full reference: https://docs.zavu.dev/concepts/sending-limits</para>
///
