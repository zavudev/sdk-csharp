...
///
/// <para>**The account must be past the unverified level to send, except on WhatsApp.**
...
/// (KYB) is not required to broadcast on any channel except `sms_oneway`, which
/// is refused with `403` and code `KYB_REQUIRED` until it is approved; KYB also gates
/// 10DLC registration. A `smart` broadcast is never refused for KYB: without it,
/// one-way SMS is simply dropped from the channels smart routing may pick for a
/// contact. A `whatsapp` broadcast is exempt: it can only be built on a template,
/// and Meta vets the business and the content when it approves that template, so
/// an unapproved template is refused instead. `smart` is not exempt, since it can
/// route a contact to SMS or email. Drafts can be created, edited and kept without
/// any check. Every send path (dashboard, API and CLI) enforces the same rule.</para>
///
