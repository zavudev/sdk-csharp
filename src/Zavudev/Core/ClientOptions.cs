...
public record struct ClientOptions()
{
    ...
    Lazy<string> _baseUrl = new(() =>
        Environment.GetEnvironmentVariable("ZAVUDEV_BASE_URL") ?? EnvironmentUrl.Production
    );
    ...
    /// <see cref="IZavudevClient.WithRawResponse"/>, don't perform validation until deserialization
    /// is triggered.</para>
    /// </summary>
    ...
    Lazy<string> _apiKey = new(() =>
        Environment.GetEnvironmentVariable("ZAVUDEV_API_KEY")
        ?? throw new ZavudevInvalidDataException(
